...
import abi from "./contracts/MKNFT.json";
import nftMetaData from "./nft_metadata.json";

function App() {
  //INSERT YOUR CONTRACT ADDRESS HERE
  const contractAddress = "0x2880E2C9d44fC41e2d66992Fb3ed756A45e2A25A";
  ...
  const checkIfWalletIsConnected = async () => {
    try {
      if (window.ethereum) {
        ...
        setOpenSeaProfile(
          `https://testnets.opensea.io/${account}?tab=activity`
        );
      } else {
  ...
  const mintToken = async (tokenId) => {
    try {
      if (window.ethereum) {
        ...
        const metadataURI = `https://gateway.pinata.cloud/ipfs/QmY9yNrBV74LC7AqeKvhh4NV4Y7U1yMxvgoGFJiyFg3Y11/${tokenId}.json`;
  ...
  return (
    <main className="container mx-auto p-4">
      <span className="prose">
        ...
        <h3 className="text-white sm:text-5xl text-3xl text-center sm:mt-5 sm:mb-10 mb-5">
          💰 Get 0.001 ETH for minting the NFT! 💰
        </h3>
      </span>
      <div className="mt-5">
        {isWalletConnected ? (
          <>
            ...
            <p className="mt-5 text-center text-white font-bold">
              {mintStatus}
            </p>
            <p className="mt-5 text-center text-amber-400	">
            ...
            <div className="mt-5">
              <p className="text-amber-400 text-center">
                <span className="font-bold text-white mt-5">
                  Your NFT will be available at:{" "}
                </span>
      ...
      <section className="customer-section pb-10 text-center">
        ...
        <div className="mt-5">
          <p className="text-amber-400">
            <span className="font-bold text-white">Contract Address: </span>
            {contractAddress}
          </p>
        </div>
        <div className="mt-5">
          <p className="text-amber-400	">
            <span className="font-bold text-white">NFT Mint Price: </span>
            {nftMintPrice} ETH
          </p>
        </div>
        <div className="mt-5">
          <p className="text-amber-400">
            <span className="font-bold text-white">NFT Supply: </span>
            {nftSupply}
          </p>
        </div>
      </section>
